...
import packages.simple_ema_bot as ema_price_bot
import pandas as pd
...
for i in range(len(chart["Close"])):
    ema_account.trade_asset(ema_price_bot.simple_ema_bot(chart["Close"][i], chart["ema_20"][i], ema_account.actual_stats(0), ema_account.actual_stats(1), i, old_price_over_ema))
    old_price_over_ema = ema_price_bot.simple_ema_bot(chart["Close"][i], chart["ema_20"][i], ema_account.actual_stats(0), ema_account.actual_stats(1), i, old_price_over_ema)[2]
